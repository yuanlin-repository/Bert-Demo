...
class BertLayerModule(nn.Module):
    ...
    def __init__(self, batch_size, max_seq_length, num_heads, hidden_size, device=torch.device('cuda'),
                 dtype=torch.float16, fused_qkv=False) -> None:
        super().__init__()
        self.device = device
        self.dtype = dtype
    ...
    def init_weights_with_bert(self, original_model, layer_idx=5):
        ...
        self.qkv_matmul_weight = torch.concat([q_weight, k_weight, v_weight], dim=0).to(torch.float16).to(
            self.device)  # [3N768, K768]
    ...
    def forward(self, src: torch.Tensor):
        if self.fused_qkv:
            ...
            q = qkv[0].reshape(self.batch_size, self.max_seq_length, self.num_heads, self.hidden_size).permute(0, 2, 1,
                                                                                                               3)
            k = qkv[1].reshape(self.batch_size, self.max_seq_length, self.num_heads, self.hidden_size).permute(0, 2, 1,
                                                                                                               3)
            v = qkv[2].reshape(self.batch_size, self.max_seq_length, self.num_heads, self.hidden_size).permute(0, 2, 1,
                                                                                                               3)
            return_t_output_qkv = torch.concat([q, k, v], dim=0)  # (batch_size * 3, num_heads, max_seq_length, hidden_size)
            t_query = torch.permute(
                torch.reshape(qkv[0], (self.batch_size * self.max_seq_length, self.num_heads, self.hidden_size)),
                (1, 0, 2))  # (num_heads, max_seq_length, hidden_size)
            t_key = torch.permute(
                torch.reshape(qkv[1], (self.batch_size * self.max_seq_length, self.num_heads, self.hidden_size)),
                (1, 2, 0))  # (num_heads, hidden_size, max_seq_length)
            t_value = torch.permute(
                torch.reshape(qkv[2], (self.batch_size * self.max_seq_length, self.num_heads, self.hidden_size)),
                (1, 0, 2))  # (num_heads, max_seq_length, hidden_size)
        else:
            t_query = torch.permute(
                torch.reshape(self.query(src),
                              (self.batch_size * self.max_seq_length, self.num_heads, self.hidden_size)),
                (1, 0, 2))
            t_key = torch.permute(
                torch.reshape(self.key(src), (self.batch_size * self.max_seq_length, self.num_heads, self.hidden_size)),
                (1, 2, 0))  # (num_heads, hidden_size, max_seq_length)
            t_value = torch.permute(
                torch.reshape(self.value(src),
                              (self.batch_size * self.max_seq_length, self.num_heads, self.hidden_size)),
                (1, 0, 2))  # (num_heads, max_seq_length, hidden_size)
        ...
        t_attn_value_output_permuted = torch.reshape(torch.permute(t_attn_value_output, (1, 0, 2)),
        ...
        t_attn_layer_norm_output = torch.layer_norm(t_attn_fc_output, (self.d_model,)).reshape(
            self.batch_size * self.max_seq_length, self.d_model)
...
class SouffleBertLayerModule(nn.Module):
    ...
    def init_weights_with_bert(self, original_model, layer_idx=0):
        factory_kwargs = {'device': self.device, 'dtype': self.dtype}
        # init with a layer
        layer = original_model.encoder.layer[layer_idx]
        attention = layer.attention.self
        ...
        self.qkv_weight = torch.stack([q_weight.t().contiguous(), k_weight.t().contiguous(), v_weight.t().contiguous()],
                                      dim=0).to(self.dtype).to(self.device)  # [3K768, N768]
        self.fc_weight = attn_output_weight.t().contiguous().to(torch.float16).to(
            self.device)  # [K768, N768] should be K * N in cuda kernel
        self.ff_fc1_weight = layer.intermediate.dense.weight.data.clone().to(torch.float16).to(
            self.device).t().contiguous()  # [K768, N3072]
        self.ff_fc2_weight = layer.output.dense.weight.data.clone().to(torch.float16).to(
            self.device).t().contiguous()  # [K3072, N768]
...
def safe_assert_close(a, b, rtol=1e-2, atol=1e-2, name="tensor"):
    try:
        torch.testing.assert_close(a, b, rtol=rtol, atol=atol)
        print(f"✅ {name}: tensors are close within tolerance.")
    except AssertionError as e:
        print(f"❌ {name}: tensors differ beyond tolerance.")
        print(e)
...
def run_bert_layer_test():
    ...
    bert_layer_module = BertLayerModule(batch_size=1, max_seq_length=384, num_heads=12, hidden_size=64, device='cuda',
                                        dtype=torch.float16, fused_qkv=True)


    souffle_bert_layer_module = SouffleBertLayerModule(batch_size=1, max_seq_length=384, num_heads=12, hidden_size=64,
                                                       device='cuda', dtype=torch.float16, fused_qkv=True)


    with (torch.no_grad()):
        bert_layer_module.init_weights_with_bert(original_model, layer_idx=0)
        t_output_qkv, t_query_key_output,  t_attn_value_output, t_attn_value_output_permuted, t_attn_fc_output_tmp, t_attn_layer_norm_output, t_ff_fc1_output, t_ff_fc2_output, t_ff_output, t_output = bert_layer_module(input_embeddings)
        souffle_bert_layer_module.init_weights_with_bert(original_model, layer_idx=0)
        output_qkv, query_key_output, attn_value_output, attn_fc_output, feed_forward_fc1_output, feed_forward_fc2_output = souffle_bert_layer_module(input_embeddings)
        torch.cuda.synchronize()
        print("---------------------layer-0---------------------\n")
        print("output_qkv:", output_qkv)
        print("query_key_output", query_key_output)
        print("attn_value_output", attn_value_output)
        print("attn_fc_output", attn_fc_output)
        print("feed_forward_fc1_output", feed_forward_fc1_output)
        print("feed_forward_fc2_output", feed_forward_fc2_output)
        print("---------------------layer-0-count-of-nan---------------------\n")
        print("output_qkv:", torch.isnan(output_qkv).sum().item())
        print("query_key_output:", torch.isnan(query_key_output).sum().item())
        print("attn_value_output:", torch.isnan(attn_value_output).sum().item())
        print("attn_fc_output:", torch.isnan(attn_fc_output).sum().item())
        print("feed_forward_fc1_output:", torch.isnan(feed_forward_fc1_output).sum().item())
        print("feed_forward_fc2_output:", torch.isnan(feed_forward_fc2_output).sum().item())
        safe_assert_close(output_qkv, t_output_qkv, rtol=1e-2, atol=1e-2)
        safe_assert_close(query_key_output, t_query_key_output, rtol=1e-2, atol=1e-2)
        safe_assert_close(attn_value_output, t_attn_value_output_permuted, rtol=1e-2, atol=1e-2)
        safe_assert_close(attn_fc_output, t_attn_layer_norm_output, rtol=1e-2, atol=1e-2)
        safe_assert_close(feed_forward_fc1_output, t_ff_fc1_output, rtol=1e-2, atol=1e-2)
        safe_assert_close(feed_forward_fc2_output, t_output, rtol=1e-2, atol=2e-2)
        print("Layer0 output match original model success!!!")
        ...
        for i in range(12):
            ...
            bert_layer_module.init_weights_with_bert(original_model, layer_idx=i)
            t_output_qkv, t_query_key_output,  t_attn_value_output, t_attn_value_output_permuted, t_attn_fc_output_tmp, t_attn_layer_norm_output, t_ff_fc1_output, t_ff_fc2_output, t_ff_output, t_output = bert_layer_module(t_output)
            souffle_bert_layer_module.init_weights_with_bert(original_model, layer_idx=i)
            output_qkv, query_key_output, attn_value_output, attn_fc_output, feed_forward_fc1_output, feed_forward_fc2_output = souffle_bert_layer_module(feed_forward_fc2_output)
            torch.cuda.synchronize()
            print("---------------------layer-", i,"---------------------\n")
            # print("output_qkv:", output_qkv)
            # print("query_key_output", query_key_output)
